...
//   }
const roadsterMongo = async () => {
	try {
		fetchItems.fetchRoadster().then((result) => {
			const roadster = new Roadster({
				earthDistanceKm: result.data.earth_distance_km,
				longitude: result.data.longitude,
			});
			Roadster.find().then((el) => {
				if (el.length >= 1) {
					const roadsterObj = Roadster.findOneAndUpdate(
						{ _id: el._id },
						{
							earthDistanceKm: result.data.earth_distance_km,
							longitude: result.data.longitude,
